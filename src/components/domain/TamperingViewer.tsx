...
export function TamperingViewer({
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-[auto,1fr]">
      {/* Image with overlay boxes drawn at fractional coordinates from module3_Tampering.anomalies[].boundingBox */}
      <div className="relative mx-auto aspect-[16/10] w-full max-w-[280px] overflow-hidden rounded-md border border-border bg-slate-950 sm:mx-0">
        {documentImage && (
          <img src={documentImage} alt="Scanned document" className="h-full w-full object-cover opacity-90" />
        )}
        {tampering.anomalies.map((anomaly, i) =>
          anomaly.boundingBox ? (
            <div
              ...
              style={{
                left: `${anomaly.boundingBox.x * 100}%`,
                top: `${anomaly.boundingBox.y * 100}%`,
                width: `${anomaly.boundingBox.w * 100}%`,
                height: `${anomaly.boundingBox.h * 100}%`,
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="font-medium text-slate-100">
            {tampering.isTampered ? "Tampering indicators found" : "No tampering detected"}
          </span>
          <span className="ml-auto font-mono text-xs text-slate-500">
            {Math.round(tampering.confidence * 100)}% confidence
          </span>
        </div>
        ...
        <ul className="space-y-1.5">
          {tampering.anomalies.map((anomaly, i) => (
            <li key={i} className="flex items-start gap-2 rounded border border-danger/30 bg-danger/10 px-2.5 py-1.5 text-xs text-slate-200">
              ...
              <span>
                <span className="font-medium">{ANOMALY_LABEL[anomaly.type] ?? anomaly.type}</span>
                {" — "}
                {anomaly.description}
              </span>
            </li>
          ...
          {tampering.anomalies.length === 0 && validation.errors.length === 0 && (
            <li className="text-xs text-slate-500">No format or tampering issues to review.</li>
          )}
        </ul>
      </div>
    </div>
